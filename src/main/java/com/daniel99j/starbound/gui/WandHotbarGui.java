...
public class WandHotbarGui extends HotbarGui {
    int ready = 3;
    int page = 0;

    public WandHotbarGui(ServerPlayerEntity player) {
        super(player);
        build();
    }
    ...
    private void build() {
        for (int i = 0; i < 4; i++) {
            if(Spells.getSpells().size() > i+4*page) {
                ...
                this.setSlot(i, GuiElementBuilder.from(s.getIcon(true)).setCallback((index, clickType, actionType, slotInterface) -> {
                    if (ready == 0) {
                ...
                ItemStack display = ItemUtils.setModelUnBridged(ItemUtils.getBasicModelItemStack(), Identifier.of(Starbound.MOD_ID, "unset_spell"));
